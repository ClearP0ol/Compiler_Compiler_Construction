...
## 项目介绍
这是一个面向教学与实践的编译器构造项目，以类 C / Tiny 语言为目标，完整覆盖了词法分析、语法分析、语义分析与中间代码生成等核心阶段。

项目采用模块化设计思想：

* 使用正则表达式 → NFA → DFA → 最小化 DFA实现可配置的词法分析器；

* 基于 SLR(1) 的自底向上语法分析框架，支持文法文件独立配置；

* 在语法分析过程中引入语法制导翻译，完成语义检查与三地址码（IR）生成；

* 支持将各阶段结果输出，便于调试、测试与教学展示。

该仓库注重可扩展性与工程规范，为后续接入 AST、LR(1)/LALR(1)、LLVM 后端或图形化界面等功能预留了清晰的演进空间。
