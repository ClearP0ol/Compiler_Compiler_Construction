...
struct LRItem {
	...
	// 相等比较
	bool operator==(const LRItem& Other) const {
		if (ProductionRef.Left.Name != Other.ProductionRef.Left.Name) return false;
		if (ProductionRef.Right.size() != Other.ProductionRef.Right.size()) return false;
		if (DotPosition != Other.DotPosition) return false;
