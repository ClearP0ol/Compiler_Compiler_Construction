...
struct ShiftReduceParser
{
	...
	void GenerateCode(int prodId, const Production& prod)
	{
		...
		if (prod.Left.Name == "Expr" && prod.Right.size() == 3 && prod.Right[1].Name == "+")
		{
			...
			SemanticAttribute op = SemanticStack.top(); SemanticStack.pop(); // 运算符，忽略
		...
		{
			SemanticAttribute rparen = SemanticStack.top(); SemanticStack.pop(); // 右括号，忽略
			SemanticAttribute expr = SemanticStack.top(); SemanticStack.pop();
			SemanticAttribute lparen = SemanticStack.top(); SemanticStack.pop(); // 左括号，忽略
			// 直接传递Expr的属性
		...
		{
			SemanticAttribute semicolon = SemanticStack.top(); SemanticStack.pop(); // 分号，忽略
			SemanticAttribute expr = SemanticStack.top(); SemanticStack.pop();
			SemanticAttribute equals = SemanticStack.top(); SemanticStack.pop(); // 等号，忽略
		...
		{
			// RelOp归约时，运算符已经在语义栈中（从移进时压入），直接传递
			// 语义栈不变，运算符属性会被传递
		}
		// 关系表达式：RelExpr -> Expr RelOp Expr
		else if (prod.Left.Name == "RelExpr" && prod.Right.size() == 3 && prod.Right[1].Name == "RelOp")
		{
			...
			SemanticAttribute relOp = SemanticStack.top(); SemanticStack.pop(); // 关系运算符
			SemanticAttribute expr1 = SemanticStack.top(); SemanticStack.pop();
			string temp = NewTemp();
			// 记录代码起始位置
			size_t codeStart = ThreeAddressCode.size();
			// 生成比较代码：temp = expr1 relOp expr2
			EmitCode(temp + " = " + expr1.Value + " " + relOp.Value + " " + expr2.Value);
			// 记录代码位置信息和表达式信息（用于while循环中重新生成）
		...
		else if (prod.Left.Name == "IterationStatement" && prod.Right.size() == 5 && prod.Right[0].Name == "while")
		{
			...
			// 生成标签
			string loopLabel = NewLabel();  // L0: 循环开始
			string endLabel = NewLabel();   // L1: 循环结束
			
			// 找到Stmt代码的起始位置（循环体代码的第一行）
			// 注意：stmt.CodeStartPos记录的是Stmt代码的第一行位置，但循环体可能包含表达式计算的临时变量
			// 我们需要找到循环体代码的真正起始位置，即第一个表达式计算的代码位置
			size_t stmtStart = 0;
			if (stmt.CodeStartPos >= 0 && stmt.Type == "stmt_start")
			{
				...
				// 我们需要找到第一个与循环体相关的代码（通常是包含循环体内变量的代码）
				// 但为了简化，我们假设循环体代码从stmt.CodeStartPos开始
				// 实际上，我们需要找到循环体代码的起始位置，这可能是表达式计算的第一个临时变量
			}
			else
			{
			...
			// 查找循环体代码的真正起始位置
			// 循环体代码应该包括所有为循环体内语句生成的代码，包括表达式计算的临时变量
			// 我们需要找到第一个与循环体相关的代码位置
			...
			string codeBefore = "";
			for (size_t i = 0; i < ThreeAddressCode.size() && i < 10; i++)
			{
				codeBefore += "[" + to_string(i) + "]" + ThreeAddressCode[i] + ";";
			}
			
			// 找到并删除原来的RelExpr代码（在循环外生成的，需要移到循环内）
			size_t relExprStart = ThreeAddressCode.size();
			if (relExpr.CodeStartPos >= 0)
			{
				relExprStart = relExpr.CodeStartPos;
			}
			else
			{
				// 如果找不到CodeStartPos，查找包含relExpr.Value的代码行
			...
			// 如果RelExpr代码在Stmt代码之后，也需要删除（虽然不应该发生，但为了安全）
			else if (relExprStart < ThreeAddressCode.size() && relExprStart >= stmtStart)
			{
				// 查找在stmtStart之前的RelExpr代码
				for (size_t i = 0; i < stmtStart && i < ThreeAddressCode.size(); i++)
				{
					if (ThreeAddressCode[i].find(relExpr.Value) != string::npos && 
					    ThreeAddressCode[i].find("=") != string::npos)
					{
						ThreeAddressCode.erase(ThreeAddressCode.begin() + i);
						if (stmtStart > i)
						{
							stmtStart--; // 调整stmtStart位置
						}
		...
		else if (prod.Left.Name == "StmtList")
		{
			...
			// 我们需要取第一个Stmt的位置，即attrs[1]（左边的Stmt），而不是attrs[0]（右边的StmtList）
			int codeStart = -1;
